...
if (($ACTION == 'EXPORT_EDIT' || $ACTION == 'EXPORT_COPY') && $STEP == 1)
{
	...
	if (isset($arOldSetupVars['SHIPMENT_OPTIONS_DAY']))
		$SHIPMENT_OPTIONS_DAY = $arOldSetupVars['SHIPMENT_OPTIONS_DAY'];
	if (isset($arOldSetupVars['SHIPMENT_OPTIONS_BEFORE']))
		$SHIPMENT_OPTIONS_BEFORE = $arOldSetupVars['SHIPMENT_OPTIONS_BEFORE'];
}
...
if ($STEP == 1)
{
	...
	if (!isset($SETUP_FILE_NAME))
		$SETUP_FILE_NAME = 'sber_'.mt_rand(0, 999999).'.php';
...
?>
<? if(CModule::IncludeModuleEx("abricos.sbermegamarket")==2) {?>
		...
		function showDetailPopup()
		{
			if (!obDetailWindow)
			{
				...
				var obDetailWindow = new BX.CAdminDialog({
					'content_url': '/bitrix/tools/abricos.sbermegamarket/sber_detail.php?lang=<?=LANGUAGE_ID?>&bxpublic=Y&IBLOCK_ID=' + s[s.selectedIndex].value,
...
<tr>
	<td width="40%"><?=GetMessage("SHIPMENT_OPTIONS_BEFORE");?></td>
	<td width="60%">
		<input type="text" name="SHIPMENT_OPTIONS_BEFORE" value="<?=$SHIPMENT_OPTIONS_BEFORE; ?>" size="5">
	</td>
</tr>

<tr>
	<td width="40%" style="padding-top: 0;">&nbsp;</td>
	<td width="60%" style="padding-top: 0;"><small><?=GetMessage("SHIPMENT_OPTIONS_BEFORE_NOTE");?></small></td>
</tr>
<tr>
	<td width="40%"><?=GetMessage("SHIPMENT_OPTIONS_DAY");?></td>
	<td width="60%">
		<input type="text" name="SHIPMENT_OPTIONS_DAY" value="<?=$SHIPMENT_OPTIONS_DAY; ?>" size="5">
	</td>
</tr>
...
if (2 > $STEP)
{
	...
	<input type="hidden" name="EXPORT_CHARSET" value="<?=LANG_CHARSET?>">
	<input type="hidden" name="ACT_FILE" value="<?echo htmlspecialcharsbx($_REQUEST["ACT_FILE"]) ?>">
	<input type="hidden" name="ACTION" value="<?echo htmlspecialcharsbx($ACTION) ?>">
	<input type="hidden" name="STEP" value="<?echo intval($STEP) + 1 ?>">
	<input type="hidden" name="SETUP_FIELDS_LIST" value="V,IBLOCK_ID,SITE_ID,SETUP_SERVER_NAME,COMPANY_NAME,SETUP_FILE_NAME,XML_DATA,USE_HTTPS,FILTER_AVAILABLE,DISABLE_REFERERS,EXPORT_CHARSET,MAX_EXECUTION_TIME,CHECK_PERMISSIONS,SHIPMENT_OPTIONS_BEFORE,SHIPMENT_OPTIONS_DAY">
