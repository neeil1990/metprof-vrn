...
use Bitrix\Main,
	...
	Bitrix\Catalog,
	Bitrix\Sale;
if (CModule::IncludeModuleEx("abricos.sbermegamarket")==2 or CModule::IncludeModuleEx("abricos.sbermegamarket")==1)
{
...
if (!isset($firstStep))
	$firstStep = true;
	$pageSize = 100;
if(CAbricosSbermegamarket::CurFile()==true)  {
	$MAX_EXECUTION_TIME =0;
	$pageSize = 20;
	}
...
$SETUP_VARS_LIST = 'IBLOCK_ID,SITE_ID,V,XML_DATA,SETUP_SERVER_NAME,COMPANY_NAME,SETUP_FILE_NAME,USE_HTTPS,FILTER_AVAILABLE,DISABLE_REFERERS,EXPORT_CHARSET,MAX_EXECUTION_TIME,CHECK_PERMISSIONS,SHIPMENT_OPTIONS_BEFORE,SHIPMENT_OPTIONS_DAY';
...
$arYandexFields = array(
	'picture','typePrefix', 'vendor', 'vendorCode', 'model',
	 'name', 'barcode','description', 'param'
);
...
if (!empty($XML_DATA['COMMON_FIELDS']) && is_array($XML_DATA['COMMON_FIELDS']))
	...
	if(is_numeric($XML_DATA['COMMON_FIELDS']['DESCRIPTION']))
	{
		$descrPropId=$XML_DATA['COMMON_FIELDS']['DESCRIPTION'];
		$descrField = 'PREVIEW_TEXT';
	}
	else
...
$selectedPriceType = 0;
$priceCode=false;
if(strpos($XML_DATA['PRICE'],'prop_')===0 or strpos($XML_DATA['PRICE'],'prop_'))
$priceCode=str_replace('prop_','',$XML_DATA['PRICE']);
...
if (!empty($XML_DATA['PRICE'])){
	...
	if ($priceCode<1)
	{
		...
		if (empty($priceType) and $priceCode<1)
			$arRunErrors[] = $XML_DATA['PRICE'].GetMessage('YANDEX_ERR_BAD_PRICE_TYPE');
		else
			$selectedPriceType = $XML_DATA['PRICE'];
		unset($priceType, $priceIterator);
	}
	elseif((int)$priceCode<1)
	{
...
if (empty($arRunErrors))
{
	...
	else
	{
		...
		unset($priceType, $priceIterator);
		if (empty($priceTypeList) and !$priceCode)
...
if ($vatExport)
{
	...
	else
	{
		...
		if (isset($vatRates[$XML_DATA['VAT_EXPORT']['BASE_VAT']]))
			$filter['!=RATE'] = $XML_DATA['VAT_EXPORT']['BASE_VAT'];
...
if (empty($arRunErrors))
{
    ...
    foreach($XML_DATA['STORE_DATA'] as $storeId=>$storeTitle)
    {
    	        if($storeTitle)
				{
					if(strpos($storeTitle,'store_')!== false)
						{
							$storeCode=str_replace('store_','',$storeTitle);
							$storeCode=(int)$storeCode;
							$itemFields[]='STORE_AMOUNT_'.$storeCode;
							$offerFields[]='STORE_AMOUNT_'.$storeCode;
						}
					elseif($storeTitle=='quant')
					{
							$itemFields[]='QUANTITY';
							$offerFields[]='QUANTITY';
					}
					else
					{
						$itemFields[]='PROPERTY_'.$storeTitle;
						$offerFields[]='PROPERTY_'.$storeTitle;
	...
	if (isset($XML_DATA['FILTER_DATA']) and isset($XML_DATA['FILTER_PREF']))
	{
     foreach($XML_DATA['FILTER_DATA'] as $key=>$val)
      {
         ...
         $pref='';
      	...
      	if(isset($XML_DATA['FILTER_PREF'][$key]) and $val!='')
      	{
      	  if($XML_DATA['FILTER_PREF'][$key]=='z')
      	      $filter["PROPERTY_".$val] = false;
      	  elseif($XML_DATA['FILTER_PREF'][$key]=='nz')
              $filter["!PROPERTY_".$val] = false;
      	  elseif($XML_DATA['FILTER_PREF'][$key]=='=')
	      	  $filter["PROPERTY_".$val.$pref] = $XML_DATA['FILTER_INPUT'][$key];
	      elseif($XML_DATA['FILTER_PREF'][$key]=='>')
	      	  $filter[">PROPERTY_".$val.$pref] = $XML_DATA['FILTER_INPUT'][$key];
      	  elseif($XML_DATA['FILTER_PREF'][$key]=='<')
	      	  $filter["<PROPERTY_".$val.$pref] = $XML_DATA['FILTER_INPUT'][$key];
      	  elseif($XML_DATA['FILTER_PREF'][$key]=='=!')
	      	  $filter["!PROPERTY_".$val.$pref] = $XML_DATA['FILTER_INPUT'][$key];
	       elseif($XML_DATA['FILTER_PREF'][$key]==1)
	      {
	      $XML_DATA['FILTER_INPUT'][$key]=str_replace(' ,',',',$XML_DATA['FILTER_INPUT'][$key]);
	      $XML_DATA['FILTER_INPUT'][$key]=str_replace(', ',',',$XML_DATA['FILTER_INPUT'][$key]);
	       $filter["PROPERTY_".$val.$pref] = explode(',',$XML_DATA['FILTER_INPUT'][$key]);
	      }
      	}
      }
	}
	
    if (!$boolOffers and $XML_DATA['FILTER_QUANTYTY'])
		$filter['>=CATALOG_QUANTITY'] = $XML_DATA['FILTER_QUANTYTY'];
	if (!$bAllSections && !empty($arSectionIDs))
	{
		$filter['INCLUDE_SUBSECTIONS'] = 'Y';
		$filter['SECTION_ID'] = $arSectionIDs;
	}
	if (!$boolOffers and $XML_DATA['FILTER_PRICE_MIN'])
    {
	    if($XML_DATA['PRICE']>0 and $priceCode<1)
			$filter['>=PRICE_'.$XML_DATA['PRICE']]  = $XML_DATA['FILTER_PRICE_MIN'];
		elseif($priceCode)
		    $filter['>=PROPERTY_'.$priceCode]  = $XML_DATA['FILTER_PRICE_MIN'];	
		else	
			$filter['>=PRICE_1']  = $XML_DATA['FILTER_PRICE_MIN'];
	}
    if (!$boolOffers and $XML_DATA['FILTER_PRICE_MAX'])
    {
	    if($XML_DATA['PRICE']>0 and $priceCode<1)
			$filter['<=PRICE_'.$XML_DATA['PRICE']]  = $XML_DATA['FILTER_PRICE_MAX'];
		elseif((int)$priceCode<0)
			$filter['<=PRICE_1']  = $XML_DATA['FILTER_PRICE_MAX'];
		else $filter['<=PROPERTY_'.$priceCode]  = $XML_DATA['FILTER_PRICE_MAX'];
    }
	$filter['ACTIVE'] = 'Y';
	...
	if ($XML_DATA['FILTER_PRICE_MIN'] and $priceCode<1)
    {
	    if($XML_DATA['PRICE']>0)
			$offersFilter['>=PRICE_'.$XML_DATA['PRICE']]  = $XML_DATA['FILTER_PRICE_MIN'];
		else
			$offersFilter['>=PRICE_1']  = $XML_DATA['FILTER_PRICE_MIN'];
	}
    if ($XML_DATA['FILTER_PRICE_MAX'] and $priceCode<1)
    {
	    if($XML_DATA['PRICE']>0)
			$offersFilter['<=PRICE_'.$XML_DATA['PRICE']]  = $XML_DATA['FILTER_PRICE_MAX'];
		else
			$offersFilter['<=PRICE_1']  = $XML_DATA['FILTER_PRICE_MAX'];
    }
    if ($XML_DATA['FILTER_QUANTYTY'])
		$offersFilter['>=CATALOG_QUANTITY'] = $XML_DATA['FILTER_QUANTYTY'];
	if ($filterAvailable)
				...
				if ($row['TYPE'] == Catalog\ProductTable::TYPE_SKU && !empty($row['OFFERS']))
				{
					...
					foreach (array_keys($row['OFFERS']) as $offerId)
					{
						...
						unset($calculatePrice);
							...
							{
							$minPrice =CAbricosSbermegamarket::GetProp($row['ID'],$IBLOCK_ID,$priceCode);	
							$fullPrice =$minPrice;
							$minPriceCurrency ='RUB';
							}
						if($XML_DATA['CORR_1'] and $XML_DATA['CORR_2'] and $XML_DATA['CORR_3'])
						{
							if($XML_DATA['CORR_1']=='+' and $XML_DATA['CORR_3']=='rur')
								 $minPrice=$minPrice+$XML_DATA['CORR_2'];
							elseif($XML_DATA['CORR_1']=='-' and $XML_DATA['CORR_3']=='rur')
								 $minPrice=$minPrice-$XML_DATA['CORR_2'];
							elseif($XML_DATA['CORR_1']=='+' and $XML_DATA['CORR_3']=='%')
								 $minPrice=$minPrice+($minPrice*$XML_DATA['CORR_2']/100);
							elseif($XML_DATA['CORR_1']=='-' and $XML_DATA['CORR_3']=='%')
								 $minPrice=$minPrice-$minPrice*$XML_DATA['CORR_2']/100;
						}
						if ($minPrice <= 0)
					...
					foreach ($row['OFFERS'] as $offer)
					{
						...
						if($fields['articulSCU'] and is_numeric($fields['articulSCU']))
							$scuScu=CAbricosSbermegamarket::GetProp($offer['ID'],$intOfferIBlockID,$fields['articulSCU']);
						elseif($fields['articulSCU']=='SCU')
						    $scuScu=$offer['XML_ID'];
						else
						    $scuScu=$offer['ID'];
					
						$itemsContent .= '<offer id="'.$scuScu.'"'.$productFormat.$available.">\n";
						unset($available);
                        unset($scuScu);

						$referer = '';

						$itemsContent .= "<url>".($offerUrlConfig['USE_DOMAIN'] ? $usedProtocol.$site['SERVER_NAME'] : '').htmlspecialcharsbx($offer['DETAIL_PAGE_URL'])."</url>\n";
						unset($referer);
						...
						if ($vatList[$offer['VAT_ID']])
							$itemsContent .= "<vat>".$vatList[$offer['VAT_ID']]."</vat>\n";
						elseif($vatRates[$XML_DATA['VAT_EXPORT']['BASE_VAT']])
	                     $itemsContent .= "<vat>".$vatRates[$XML_DATA['VAT_EXPORT']['BASE_VAT']]."</vat>\n";
						...
						foreach ($arYandexFields as $key)
						{
							...
							{
								case 'name':
										if(!empty($XML_DATA['TITLE']))
										{
											$title=CAbricosSbermegamarket::GetProp($row['ID'],$IBLOCK_ID,$XML_DATA['TITLE']);
										}
										if($title)
										 $itemsContent .= "<name>".yandex_text2xml($title, $itemOptions)."</name>\n";
										elseif(!empty($XML_DATA['SBER_TITLE_TEMPL']))
										{
											$itemsContent .= "<name>".yandex_text2xml(CAbricosSbermegamarket::get_text($XML_DATA['SBER_TITLE_TEMPL'],$row['NAME'],$sectArr[$row['CATEGORY_ID']],$offer['NAME']), $itemOptions)."</name>\n";
										}
										else
								...
								if($descrPropId)
								{
									$descr=CAbricosSbermegamarket::GetProp($row['ID'],$IBLOCK_ID,$descrPropId);
									if($descr!='')
									 $descr=yandex_text2xml($descr, $itemOptions);
								}
								else $descr=($offer['DESCRIPTION'] !== '' ? $offer['DESCRIPTION'] : $row['DESCRIPTION']);

								if(!empty($XML_DATA['SBER_DESCR_TEMPL']))
								{
									$descr2=yandex_text2xml(CAbricosSbermegamarket::get_text($XML_DATA['SBER_DESCR_TEMPL'],$row['NAME'],$sectArr[$row['CATEGORY_ID']],$offer['NAME']), $itemOptions);
								}
								if(!empty($XML_DATA['SBER_DESCR_TEMPL']) and $XML_DATA['SBER_DESCR_TEMPL_PL']=='top')
									$itemsContent .= "<description>".$descr2.$descr."</description>\n";
								elseif(!empty($XML_DATA['SBER_DESCR_TEMPL']) and $XML_DATA['SBER_DESCR_TEMPL_PL']=='top')
									$itemsContent .= "<description>".$descr.$descr2."</description>\n";
								elseif(!empty($XML_DATA['SBER_DESCR_TEMPL']) and $XML_DATA['SBER_DESCR_TEMPL_PL']=='none')
									$itemsContent .= "<description>".$descr2."</description>\n";
								else
								$itemsContent .= "<description>".$descr."</description>\n";
									break;
								case 'param':
									if ($parametricFieldsExist)
									{
								...
								if ($fieldsExist && isset($fields[$key]))
								{
									...
									$value=str_replace('<picture>','',$value);
									$value=str_replace('</picture>','',$value);
									$pict=explode(',',$value);
									foreach($pict as $pic)
									{
										if($pic!='')
											$itemsContent .='<picture>'.trim($pic)."</picture>\n";
		                            }
									unset($value);
									unset($pic);
									unset($pict);
	                         ...
	                         $storeQuant=0;
	                    	foreach($XML_DATA['STORE_INPUT'] as $storeId=>$storeTitle)
									{
										if(!empty($storeTitle))
									     {
					                        if($XML_DATA['STORE_DATA'][$storeId]=='quant')
											  $storeQuant=$offer['QUANTITY'];
											 elseif(strpos($XML_DATA['STORE_DATA'][$storeId],'store_')!== false)
											  {
											  	$storeCode=str_replace('store_','',$XML_DATA['STORE_DATA'][$storeId]);
												$storeQuant=$offer['STORE_AMOUNT_'.intval($storeCode)];
											  }
											else
											  $storeQuant=(int)$offer['PROPERTY_'.$XML_DATA['STORE_DATA'][$storeId].'_VALUE'];
                                             if(!$storeQuant or $storeQuant<0 or $storeQuant=='')
                                               $storeQuant=0;
											  $itemsContent .= "<outlet instock='".$storeQuant."' id='".$storeTitle."'></outlet>\n";
										  }
									}
						$itemsContent .= "</outlets>";
						$itemsContent .= '</offer>'."\n";
...
if($priceCode<1)
					...
					if ($minPrice <= 0)
						continue;
					if(($XML_DATA['FILTER_PRICE_MAX']>0 and $minPrice>$XML_DATA['FILTER_PRICE_MAX']) or ($XML_DATA['FILTER_PRICE_MIN']>0 and $minPrice<$XML_DATA['FILTER_PRICE_MIN']))
					    continue;
                    if(empty($row['DESCRIPTION']) and $SBER_NODESCR=='Y')
	                     continue;
	                if(empty($row['PICTURE']) and $SBER_NOPHOTO=='Y')
	                     continue;
					if (is_numeric($XML_DATA['FILTER_QUANTYTY']) and $row['QUANTITY']<=$XML_DATA['FILTER_QUANTYTY'])
							 continue;
						 $available = ' available="'.($row['AVAILABLE'] == 'Y' ? 'true' : 'false').'"';
					
                    if($fields['articul'] and is_numeric($fields['articul']))
					    $scu=CAbricosSbermegamarket::GetProp($row['ID'],$IBLOCK_ID,$fields['articul']);
				    elseif($fields['articul']=='SCU')
						$scu=$row['XML_ID'];
					else
						$scu=$row['ID'];
						
					$itemsContent .= '<offer id="'.$scu.'"'.$productFormat.$available.">\n";
					unset($available);
                    unset($scu);

					$referer = '';

					$itemsContent .= "<url>".($itemUrlConfig['USE_DOMAIN'] ? $usedProtocol.$site['SERVER_NAME'] : '').htmlspecialcharsbx($row['DETAIL_PAGE_URL'])."</url>\n";
					unset($referer);
					...
					foreach ($arYandexFields as $key)
					{
						switch ($key)
						{
							case 'name':
										...
										elseif(!empty($XML_DATA['SBER_TITLE_TEMPL']))
										{
											 $itemsContent .= "<name>".yandex_text2xml(CAbricosSbermegamarket::get_text($XML_DATA['SBER_TITLE_TEMPL'],$row['NAME'],$sectArr[$row['CATEGORY_ID']]), $itemOptions)."</name>\n";
										}
										else
							...
							case 'description':
								if($descrPropId)
								{
									$descr=yandex_text2xml(CAbricosSbermegamarket::GetProp($row['ID'],$IBLOCK_ID,$descrPropId), $itemOptions);
									if($descr!='')
									 $descr=yandex_text2xml($descr, $itemOptions);
									}
								else $descr=$row['DESCRIPTION'];
								if(!empty($XML_DATA['SBER_DESCR_TEMPL']))
								{
									$descr2=yandex_text2xml(CAbricosSbermegamarket::get_text($XML_DATA['SBER_DESCR_TEMPL'],$row['NAME'],$sectArr[$row['CATEGORY_ID']]), $itemOptions);
								}
								if(!empty($XML_DATA['SBER_DESCR_TEMPL']) and $XML_DATA['SBER_DESCR_TEMPL_PL']=='top')
									$itemsContent .= "<description>".$descr2.$descr."</description>\n";
								elseif(!empty($XML_DATA['SBER_DESCR_TEMPL']) and $XML_DATA['SBER_DESCR_TEMPL_PL']=='bottom')
									$itemsContent .= "<description>".$descr.$descr2."</description>\n";
								elseif(!empty($XML_DATA['SBER_DESCR_TEMPL']) and $XML_DATA['SBER_DESCR_TEMPL_PL']=='none')
									$itemsContent .= "<description>".$descr2."</description>\n";
								else
										...
										if(!empty($storeTitle))
									     {
					                        if($XML_DATA['STORE_DATA'][$storeId]=='quant')
											 ...
											 elseif(strpos($XML_DATA['STORE_DATA'][$storeId],'store_')!== false)
											  {
											  	$storeCode=str_replace('store_','',$XML_DATA['STORE_DATA'][$storeId]);
												$storeQuant=$row['STORE_AMOUNT_'.intval($storeCode)];
											  }
											else
											  $storeQuant=(int)$row['PROPERTY_'.$XML_DATA['STORE_DATA'][$storeId].'_VALUE'];
                                             if(!$storeQuant or $storeQuant<0 or $storeQuant=='')
                                               $storeQuant=0;
											  $itemsContent .= "<outlet instock='".$storeQuant."' id='".$storeTitle."'></outlet>\n";
										  }
		...
		if(CAbricosSbermegamarket::CurFile()) break;
	}
...
if (empty($arRunErrors))
{
	...
	if ($finalExport)
	{
		...
		$content .= "</categories>\n";
		$content .= "<shipment-options>\n
<option days='".$SHIPMENT_OPTIONS_DAY."' order-before='".$SHIPMENT_OPTIONS_BEFORE."'/>\n
</shipment-options>\n";
		$content .= "<offers>\n";
...
$arRunErrors[] = GetMessage('ABRICOS_ERR_DEMO');
