...
foreach ($arResult['ITEMS'] as $key => &$arItem){
  ...
  $orignSec = $arItem["IBLOCK_SECTION_ID"];

  $res = CIBlockElement::GetByID($arItem["ID"]);
  if($ar_res = $res->GetNext()){

    $orignSec = $ar_res["IBLOCK_SECTION_ID"].'<br>';

  }


  $res = CIBlockSection::GetByID($orignSec);
  if($ar_res = $res->GetNext()){

     if($ar_res['CODE'] != $arParams["SECTION_CODE"]){
         $arResult['ITEMS'][$key]['DETAIL_PAGE_URL'] = $ar_res["SECTION_PAGE_URL"].$arItem["CODE"].'/';
     }
  }
  if($arItem['PROPERTIES']['NAME_SEO']['VALUE']){
      $arResult['ITEMS'][$key]['NAME'] = preg_replace('#(~(.*?)~)#is', '', $arItem['PROPERTIES']['NAME_SEO']['VALUE']);
  }else{
